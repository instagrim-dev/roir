...
| 9 | Oracle **under-fit** for its declared behavior class, or a VT with no nameable behavior class in `proves` | Rows 1–8 are helper-*mechanics* defects; this one is a **fitness** defect. A `verification_target` whose oracle pattern cannot falsify its declared behavior class (a smoke oracle for a refactor-equivalence or regression claim; an exact-match oracle for a relation-only claim) substantiates nothing even when it exits 0 — and a VT whose `proves` names no behavior class cannot be judged for fit at verify. Fitness is reviewer-judged, not helper-checked. | Choose the minimum-fit oracle for the class per the behavior-class → minimum-fit map in [`references/oracle-patterns.md`](../references/oracle-patterns.md), and make the behavior class nameable from `proves` (no new schema field). If none of the seven classes fit and no metamorphic/differential oracle is constructible, defer the unit on a recorded no-fit note rather than passing it on smoke. |
